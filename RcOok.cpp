...
int OregonDecoderV2::decode (word width) {
	if (200 <= width && width < 1200) {
		...
		return ( total_bits == 176 )? 1 : -1;
	}
	if( total_bits >= 160 ) {
		unsigned short int sensor = 0;
...
int RCSwitch_::decode (word duration) {
	...
	if ( changeCount > 6 && duration > 5000 && duration > this->timings[0] - 200 && duration < this->timings[0] + 200) {
		/* this looks like end */
		...
		/* otherwise it's just the begining */
		changeCount = 0;
	}
